#pragma once

#ifndef __MAP_STAGE2BOSSROOM_H__
#define __MAP_STAGE2BOSSROOM_H__

#include "GameObject.h"

USING(Engine)

class CMap_Stage2BossRoom final : public CGameObject
{
private:
	enum EMapType
	{
		BossRoomCircle,
		BossRoomFloor1,
		BossRoomFloor2,
		BossRoomGrill,
		BossRoomInFance,
		BossRoomOutFance,
		BossRoomPillar1,
		BossRoomPillar2,
		BossRoomRock,
		BossRoomStair,
		BossRoomEntrance,
		BossRoomEntranvveFloor,
		BossRoomWall,
		End
	};

private:
	explicit CMap_Stage2BossRoom(_DEVICE _pDevice);
	explicit CMap_Stage2BossRoom(const CMap_Stage2BossRoom& _rOther);
	virtual ~CMap_Stage2BossRoom() = default;

protected:
	virtual HRESULT Ready_GameObject_Prototype() override;
	virtual HRESULT Ready_GameObject_Clone(const _uint& _iSceneIdx, void * _pArg = nullptr);
public:
	virtual _uint Update_GameObject(const _float & _fDeltaTime) override;
	virtual HRESULT Render_GameObject() override;

private:
	HRESULT Add_Component();
	HRESULT SetUp_ConstantTable(_EFFECT _pEffect);
	HRESULT Ready_MapCollider();

public:
	void Add_RenderList();

public:
	_uint Get_StageMeshCount() { return EMapType::End; }
	CStaticMesh* Get_StageMesh(_uint _iIndex) { return m_pMesh[_iIndex]; }

private:
	HRESULT Clone_Distortion();
public:
	static CMap_Stage2BossRoom* Create(_DEVICE _pDevice);
	virtual CGameObject * Clone(const _uint& _iSceneIdx, void * _pArg = nullptr);
	virtual void Free() override;

private:
	class CStaticMesh*			m_pMesh[EMapType::End];
	class CTransform*			m_pTransform;
	class CShader*				m_pShader;
	class CEffect_Texture*		m_pDistortion;

	TRANSFORM_DESC				m_tTransform;

};

#endif // !__MAP_STAGE2BOSSROOM_H__