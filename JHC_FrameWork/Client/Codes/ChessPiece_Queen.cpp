...
#pragma region Check_PlayerDead
	...
	//if (!(Get_EventMgr()->Get_CurEvent() & EVENT_VOLATILE_STAGE_CHESS) && EChessState::Idle == m_eCurChessState)	// 체스 종료?
	//{
	//	m_eCurChessState = EChessState::Dead;
	//}

	if (Get_EventMgr()->Get_CurEvent() & EVENT_VOLATILE_STAGE_CHESSQUEEN)	// 체스 종료?
	{
		m_eCurChessState = EChessState::Dead;
	}

	if (EChessState::Ready != m_ePreChessState && !(Get_EventMgr()->Get_CurEvent() & EVENT_VOLATILE_STAGE_CHESSSTART))	// 체스 종료?
	{
		m_eCurChessState = EChessState::Dead;
	}

	if (Get_EventMgr()->Get_CurEvent() & EVENT_VOLATILE_STAGE_CHESSSTART && EChessState::Ready == m_ePreChessState)	// 체스 시작
	{
