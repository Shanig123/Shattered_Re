...
static const float texscaler = 0.0002f;
...
PS_OUT		PS_MAIN_HDR(PS_IN In)
{
	...
	if(vDistort1.a > 0.f)
	{
		vUV.xy += vDistort1.rg*0.01f;
		//vNoise*= vDistort1.rgb*0.01f;
	}	
	if (vDistort2.a > 0.f)
	{
		//vUV = In.vTexUV;
		vUV.xy += vDistort2.rg*0.01f;
	...
	Out.vColor.rgb = (( vShade.rgb + vSpecular.rgb)* vShadow.rgb + vEmissive.rgb)/** vNoise*/;
	Out.vColor.a = 1.f;
