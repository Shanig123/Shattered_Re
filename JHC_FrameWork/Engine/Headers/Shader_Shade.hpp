...
PS_OUT			PS_DIROREN(PS_IN In)
{
	...
	//vAlbedo = 1.f;
	float	fSpecular = pow(abs(tex2D(DepthSampler, In.vTexUV).g),2.2f);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	Out.vShade.rgb = PS_Oren_Nayer(vNormal,
		...
		fSpecular,
		NoL);
	vector vFog = Ps_Fog(vAlbedo, fViewPosZ, In.vTexUV);
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if ((fRough <= 0 && fSpecular <= 0) /*|| fRough > 0 && fSpecular <= 0*/)
		Out.vSpecular.rgb = PS_SpecularS(fRough, NoH, NoL, NoV, fSpecular, vAlbedo);
		//Out.vSpecular.rgb = 1.f;
	else if (fRough <= 0 && fSpecular > 0)
		Out.vSpecular.rgb = ((PS_SpecularF(fSpecular, vAlbedo, NoL)/**vFog*//**PS_Environment(reflect(g_vLightDir, vNormal).rgb)*/) / (4 * NoV*NoL));
	else
		Out.vSpecular.rgb = (PS_SpecularS(fRough,NoH,NoL,NoV, fSpecular, vAlbedo)/**vFog*//**PS_Environment(reflect(g_vLightDir, vNormal).rgb)*/);
...
PS_OUT			PS_POINTOREN(PS_IN In)
{
	...
	vCamEye = normalize(g_vCamPos - vPosition);
	vector vInvLightDir = normalize(-vLightDir);
	...
	float NoL)
{
	...
	return (vAlbedo.rgb*(max(0.0f, NoL))*fABC)/**(g_fPower/pow(g_fRange,2))*/;
}
...
float3 PS_SpecularF(float fSpecular, vector vAlbedo, float NoL)
{
	...
	*/
	...
	float3 vSpecular = fSpecular;
	return	vSpecular = lerp(0.08 * vSpecular.xxx, vAlbedo.rgb, vSpecular.xxx);
	//R0
