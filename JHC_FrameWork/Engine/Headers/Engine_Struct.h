...
typedef struct tagParticlePatternDesc
{
	tagParticlePatternDesc()
		...
		ePatternType((_ulong)ePartilcePatternType::End)
	{
	}
...
{
	tagTrailDesc() { 
		ZeroMemory(this, sizeof(tagTrailDesc));
		eRenderState = ERenderPriority::NoBlur;
		iRenderPass = 4;
		iRenderID_Filter = 0;
	}
	...
	ERenderPriority	eRenderState;
	void*		pObj;					//부모 오브젝트
	_uint		iRenderID;				//트레일 출력위한 텍스쳐id
	_uint		iRenderID_Filter;				//트레일 출력위한 텍스쳐id
	_uint		iRenderPass;
	_bool		bRenderOnOff;
	_bool		bPartilceOnOff;
}TRAIL_DESC;
