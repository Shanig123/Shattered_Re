...
struct PS_OUT_NOBLUR
{
	vector		vEmissive : COLOR0;
	//vector      vEmissive : COLOR1;
	//vector		vNormal : COLOR2;
	//vector		vDepth : COLOR3;
	//float		vShadow : DEPTH;
...
PS_OUT_NOBLUR			PS_MAIN_Effect_NOBLUR(PS_IN_Effect In)
{
	//PS_OUT		Out = (PS_OUT)0;
	//Out.vColor = 0.f;
	//vector vEmissive;
	...
	PS_OUT_NOBLUR		Out = (PS_OUT_NOBLUR)0;
	Out.vEmissive = 0.f;
...
PS_OUT_NOBLUR	PS_MAIN_Effect_Color(VS_OUT_COLOR In)
{
	PS_OUT_NOBLUR		Out = (PS_OUT_NOBLUR)0;
	...
	// = tex2D(EmissiveSampler, float2(In.vTexUV.x+ length(tex2D(DistortionSampler, In.vTexUV + g_ftime).rgb), In.vTexUV.y));
	vEmissive = tex2D(EmissiveSampler, In.vTexUV);
...
technique Default_Device
{
	...
	pass NoBlurGauss	//5
	{
		...
		VertexShader = compile vs_3_0 VS_MAIN_Effect();
		PixelShader = compile ps_3_0 PS_MAIN_Effect_NOBLUR();
	}
};
